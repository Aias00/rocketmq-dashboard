...
class AlertNotificationSuppressionServiceTest {
    ...
    @Test
    void findsAnActiveClusterIncidentInTheSameResourceScope() {
        AlertRepository repository = mock(AlertRepository.class);
        LocalDateTime now = LocalDateTime.now();
        SystemAlertVO cause = event(1L, AlertDomain.CLUSTER, "FIRING", "broker-1", now.minusHours(2));
        ...
        verify(repository).findAlertsPage(org.mockito.ArgumentMatchers.argThat(query -> query.from() == null
                && query.to().equals(now) && query.domain() == AlertDomain.CLUSTER));
    }

    @Test
    void doesNotSuppressAfterTheSameClusterIncidentHasResolved() {
        AlertRepository repository = mock(AlertRepository.class);
        LocalDateTime now = LocalDateTime.now();
    ...
    @Test
    void doesNotSuppressAcrossDifferentBrokerScopes() {
        AlertRepository repository = mock(AlertRepository.class);
        LocalDateTime now = LocalDateTime.now();
